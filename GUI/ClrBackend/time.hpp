...
struct mytime {
	...
	mytime &operator= (const std::string &t) {
		date = 0;
		if (t[0] == 'X') {
			hr = mn = 0;
			return *this;
...
struct mydate {
	...
	operator std::string() {
		std::string ret;
		ret += std::to_string(yr); ret += '-';
		if (mn < 10) ret += '0'; ret += std::to_string(mn); ret += '-';
		if (dy < 10) ret += '0'; ret += std::to_string(dy);
		return ret;
	}
