...
public:
    ...
    bool operator() (const tstring<s1> &a, const tstring<s2> &b) {
        int tlen = std::min(a.getLen(), b.getLen());
        for (int i = 0; i < tlen; i++) {
...
std::string fill_to(std::string b, int tar) {
    std::string ret = b;
